...
const upload = multer({});
let dataPathName = join(__dirname, "./users.json");
let photoPathName = join(__dirname, "../uploads/avatars/");
router.get("/:id", async (req, res, next) => {
  ...
  if (id) {
    ...
    if (result !== -1) {
      ...
      let error = new Error(`I couldn't find the user which has ${id}`);
...
router.post("/", async (req, res, next) => {
  ...
  try {
    if (body === {}) {
      ...
      let updated = await append(dataPathName, req.body, "user");
      res.send(updated);
    }
  } catch (error) {
    next(error);
...
router.put("/:id/photo", upload.single("photo"), async (req, res, next) => {
  try {
    ...
    let url = `http://localhost:3003/photos/avatars/${id}.${extension}`;
